...
C
      IMPLICIT NONE
      INTEGER LPAWC,MPAWC
      PARAMETER (LPAWC=500000)
      COMMON/PAWC/ MPAWC(LPAWC)
C
...
C
      ...
      lrec=1024      
C
      CALL HROPEN(1,'HOUT','m_opt.his','N',lrec,ista)
      IF(ista.NE.0) THEN
         WRITE(6,*) ' *** HROPEN error'
         GO TO 999
      ENDIF
C
      CALL M_OINI(dc(1))
...
C
      ...
      DO i=1,3
         IF(i.LE.1) THEN
            bst(i)=(bmnx(2,i)-bmnx(1,i))/REAL(nstep1)
         ELSE
            bst(i)=(bmnx(2,i)-bmnx(1,i))/REAL(nstep2)
         ENDIF
      END DO
      IF(NBINY.GT.0) THEN
         CALL HBOOK2(ID+1,'Thetamax vs Q1(X),Q2(Y), coincidence'
     +     ,nstep1,bmnx(1,1),bmnx(2,1),nstep2,bmnx(1,2),bmnx(2,2),0.)
         CALL HCOPY(1+ID,2+ID,'Q3 vs Q1(X),Q2(Y), coin')
         CALL HCOPY(1+ID,3+ID,'Phi vs Q1(X),Q2(Y), coincidence')
         CALL HCOPY(1+ID,4+ID,'Weight')
         CALL HCOPY(1+ID,5+ID,'Thetmax vs Q1(X),Q2+Q3(Y), coin')
         CALL HCOPY(1+ID,6+ID,'Q2 vs Q1(X),Q2+Q3(Y), coin')
         CALL HBOOK2(ID+7,'Thet angle(X) vs Sum of Q1-Q3 '
     +               ,40,0.,0.4,40,0.,10.,0.)
         CALL HCOPY(1+ID,9+ID,'Sum of the fields in Quads')
         CALL HCOPY(1+ID,10+ID,'Sum of the ABS(fields) in Quads')
      ENDIF
      CALL HBOOK1(ID+20,'thetmx vs Q1',nstep1,bmnx(1,1),bmnx(2,1),0.)
      CALL HCOPY(ID+20,ID+19,'Q1 vs Q1 at thetmx')
      CALL HCOPY(ID+20,ID+21,'Q2 vs Q1 at thetmx')
      CALL HCOPY(ID+20,ID+22,'Q3 vs Q1 at thetmx')
      CALL HCOPY(ID+20,ID+23,'Q1-3 vs Q1 at thetmx')
      CALL HCOPY(ID+20,ID+24,'d(thet)/dQ1 vs Q1 at thetmx')
      CALL HCOPY(ID+20,ID+25,'d(thet)/dQ2 vs Q1 at thetmx')
      CALL HCOPY(ID+20,ID+26,'ABS(Q1-3) vs Q1 at thetmx')
      nb2=20
      IF(NBINY.GT.0) THEN
         CALL HBOOK2(ID+27,'ctet vs Q1(X),Q1(Y), Q2,Q3 at max'
     +     ,nstep1,bmnx(1,1),bmnx(2,1),nb2,-nb2*0.01/2.,nb2*0.01/2.,0.)
         CALL HCOPY(ID+27,ID+28,'ctet vs Q1(X),Q2(Y), Q3,Q3 at max')
         CALL HCOPY(ID+27,ID+29,'ctet vs Q1(X),Q3(Y), Q2,Q2 at max')
      ENDIF
C
C===    Dipole settings
C
...
C      
      DO i1=1,nstep1
         bcur(1)=bmnx(1,1)+(i1-0.5)*bst(1)
         IF(NBINX.EQ.-1) THEN
            IF(EBEAM.LT.5.9) THEN
               bcur(1)= 0.515+1.3046*EBEAM-0.26062*EBEAM**2
            ELSE
               bcur(1)=12.698-3.5253*EBEAM+0.20862*EBEAM**2
            ENDIF
         ENDIF
C
         ct1max=0.
         DO i2=1,nstep2
            IF(nstep2.GT.1) THEN
               bcur(2)=(bmnx(1,2)+(i2-0.5)*bst(2))
            ELSE
               IF(EBEAM.LT.2.0) THEN
                  bcur(2)=0.
               ELSE
                  bcur(2)=2.2*(EBEAM-2.0)/6.
               ENDIF
            ENDIF
            bcur(3)=0.
C
...
C
            ...
            ENDIF
C
            CALL HF2( 1+ID,bcur(1),bcur(2),ctmax)

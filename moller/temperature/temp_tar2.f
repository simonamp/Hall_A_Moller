...
C
      ...
      PARAMETER (mxx=10,mxy=10,mxt=100000,mxbeam=400000,mxd=10,mxdc=8)
...
C                                  =0 normal cell, =1 - temperature zero, =2 - no thermal contact
      ...
      INTEGER*1 iadjd(4,mxdi,mxdi) ! status of adjacent cells for the plug =0 - fine cell, =1 - zero temp, =2 - no termal contact, 
C                                     =3 - coarse cell
      DOUBLE PRECISION                     ! area with fine division 
     +                 tdiv(mxdi,mxdi)     ! temperature of the fine grid, at the end of an iterration
     +                ,tdivadd(mxdi,mxdi)  ! temperature increments of the fine grid, at the end of an iterration
     +                ,tdivadd0(mxdi,mxdi) ! temperature increments of the fine grid, from the previous step
...
C                          RMS(X)=RMS(y)=1mm (from wire array measurements) E158
      DATA dtv/100.D-6/     ! time step for writing temperature into ttim 
...
C
      ...
      ipri=0
      xsiz=1.         !  x-size of the foil
      ysiz=1.         !  y-size of the foil
...
C
      DO ix=1,mxx
         DO iy=1,mxy
            ...
            ENDDO
         ENDDO
         iadjc(4,1  ,ix)=1 ! 2 0 deg at the rim
         iadjc(2,mxy,ix)=1 ! 2
      ENDDO
      DO iy=1,mxy
         iadjc(3,iy,1  )=1
      ...
      DO ixd=1,mxdi
         DO iyd=1,mxdi
            ...
            ENDDO
         ENDDO
         iadjd(2,mxdi,ixd)=1 ! 3 
         iadjd(4,1   ,ixd)=1 ! 3 
      ENDDO
      DO iyd=1,mxdi
         iadjd(1,iyd,mxdi)=1 ! 3 
         iadjd(3,iyd,1   )=1 ! 3 
      ENDDO
      IF(ipri.NE.0) THEN

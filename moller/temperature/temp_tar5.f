...
C
     ...
     +    ,irastr 
     +    ,ixss,iyss,ixyb(2),ispl,ixytmax(2)
...
C
      ...
      PARAMETER (mxdx=40,mxdcx=4,mxdy=40,mxdcy=4)
...
C
      DO ixd=1,mxdix
         DO iyd=1,mxdiy
            ...
            IF(tdiv(iyd,ixd).GT.tempmax) THEN
               tempmax=tdiv(iyd,ixd)
               ixytmax(1)=ixd
               ixytmax(2)=iyd
            ENDIF
         ENDDO
...
C
      IF(MOD(nt,1).EQ.0)
     +    WRITE(6
     +     ,FMT='(A16,I8,E11.3,F13.7,3F14.5,2F8.4,2I4,F14.5,2X,2I4)') 
     +    ' iter, time',nt,dt,tim0,tdiv(iydc,ixdc),tdivadd(iydc,ixdc),qq
     +     ,xybeam,ixyb,tempmax,ixytmax
...
C
      ...
      WRITE(9,FMT='(I7,2X,F14.7,5X,F13.7,4X,2I6)') 
     +          (i,ttim(i),ttempmax(i),ixytmx(1,i),ixytmx(2,i),i=1,ntv)
      CLOSE(9)
C
 999  CONTINUE
      END

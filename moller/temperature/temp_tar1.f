...
C
     ...
     +    ,bcurr,pulses
     +    ,dtmax  ! maximum time interval, calculated from the beam energy release (temp increase<1 deg)
C
      INTEGER    mxx,mxy,mxt,mxbeam,mxd,mxdc,mxdi
      PARAMETER (mxx=30,mxy=30,mxt=100000,mxbeam=400000,mxd=100,mxdc=4)
...
C
      ...
      DATA bcurr/0.5E-6/   ! beam current, uA 
C      DATA pulse/3.6E11/   ! e-/pulse 
      DATA ang/90./        ! beam angle, deg (20 deg)
      DATA depos/2.4E-13/  ! energy deposit by one particle J/(g/cm**2)
      DATA rb/0.01/        ! beam density: dN/ds = pulse/(2*pi*rb**2)*exp(-r**2/2/rb**2)
...
C                          RMS(X)=RMS(y)=1mm (from wire array measurements) E158
      DATA dtv/1.E-6/     ! time step for writing temperature into ttim 
      DATA fpulse/500.E6/   ! pulse repetition rate
C
      TIM=2000.E-6
      fradiat=0.0    ! radiation power: thematerial factor (0.1 is a guess)
      troom=300.            ! room temperature
      ipri=0
      xsiz=3.       !  x-size of the foil
...
C            write(6,*) ixd,iyd,fac,scal,dtmp
      ...
      dener=0.
C
      dtmax=0.1/dtmpmx   ! time interval - maximum 1 deg temperature rise
C
      DO ixd=1,mxdi
         DO iyd=1,mxdi
            dtmp=tdiv(iyd,ixd)
...
C         IF(MOD(npulses,MAX(INT(dtv/tpulse),1)).EQ.0) THEN
      IF(ntv.LT.mxt) THEN
         ntv=ntv+1
         ttim(ntv)=tim0
...
C
      fac=ak/rho/cv*dtmax
      facr=2.*fradiat/zsiz/rho/cv*dtmax
      dtmpmx=0.
C
      dener=0
...
C
      DO ixd=1,mxdi
         DO iyd=1,mxdi
            tdiv(iyd,ixd)=tdiv(iyd,ixd)+tdiv0(iyd,ixd)*dt/dtmax
         ENDDO
      ENDDO
C
      DO ix=1,mxx
...
C
               tt(iy,ix)=tt(iy,ix)+tt0(iy,ix)*dt/dtmax
            ENDIF
         ENDDO
      ENDDO
      WRITE(6,FMT='(A30,I6,E12.3,F13.7,7F12.3,2X,F8.4)') 
     +  ' iter,dt,tim0,tt,tt1,ener ',nt,dt,tim0
     +         ,(tdiv(iydc+i,ixdc),i=-3,3),dener
...
C
      IF(MOD(nt,1000).EQ.0) 
     +    WRITE(6,FMT='(A16,I8,F12.6,F14.3,2F9.4)') 

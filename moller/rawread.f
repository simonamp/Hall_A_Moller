...
C
      ...
      INTEGER*2 i2(2),j2(2)
      BYTE      i1(4),j1(4)
      EQUIVALENCE (i4,i2(1),i1(1)),(j4,j2(1),j1(1))
C
      fnam='raw.dat'//CHAR(0)
...
C
      ...
      WRITE(6,*) 'istat,ihandle=',istat,ihandle
      IF(istat.NE.0) THEN
...
C
      ...
      IF(nrec.LT.20) THEN
        WRITE(6,*) 'Event ',nrec
        WRITE(6,FMT='(8(2X,Z9))') (ibuf(i),i=1,nwor)
      ENDIF
C
      IF(nwor.LT.1) THEN
...
C
         ...
         ENDIF
C
C---           Readout controller data bank (may be several)?
C
...
C
         ...
         IF(lenn.GT.0) THEN
C
            IF(IROCFRAG(IFRAG).EQ.7) CALL MOLDECEV(lenn,ibuf(icur+1))
C
            icur=icur+lenn
C
         ENDIF
...
C
      ...
      CALL HBNAME(id,'run',IRUNCUR,'irun[1,16000]')
        ...
        IF(NTDC.GT.0) THEN
           ...
           DO i=1,NTDC
              ...
              IF(j.GE.1.AND.j.LE.5) THEN
                 IF(IABS(ITIM(i)-150).LT.10) IHIT(j)=ITIM(i)
              ENDIF

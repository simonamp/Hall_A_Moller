...
C============= Counters
     ...
     +       ,khelshift            ! =1 for runs <12700, =0 - after. The helicity of the
C                                     scaler trigger was read AFTER the proper window (shift =+1)
C                                     Since Apr 2008 the MPS signal comes 25us earlier, and the
C                                     helicity flag belongs to the proper window, then 25us was reduced to 1
     +       ,kqrtshift            ! =1 for runs <14100, =0 - after. The qrt of the
C                                     scaler trigger was read AFTER the proper window (shift =+1)
C                                     Since March 2010 the QRT signal comes at the same time as helicity, 1us
C                                     later than the MPS
...
C Oct 18, 2010 R.P. for Helicity freq = 943Hz ISCA(12)=85 (<100)
C         IF(itickev.EQ.jticksc(istack).AND.
C     +      ISCA(12)-jsca(12,istack).LT.100) THEN
         IF(itickev.EQ.jticksc(istack).AND.
     +      ISCA(12)-jsca(12,istack).LT.75) THEN
...
C
      ...
      ispqrt=isprev
      IF(kqrtshift.EQ.0) ispqrt=istack
C
...
CC   31 March 2010 Correction for 240Hz (removed)
CC            IF(itickev-itickadcf-DTICK.GT.4.) ierr=1

C            IF(itickev-itickadcl.LT.2) ierr=1
         ENDIF
         IF(isprev.GT.0) THEN
            IF(ABS(itickev-jticksc(isprev)-DTICK).GT.2.) ierr=1
CC   31 March 2010 Correction for 240Hz
CC            IF(ABS(itickev-jticksc(isprev)-DTICK).GT.4.) ierr=1 
         ENDIF
         IF(ierr.NE.0) THEN
            KVERR(3)=KVERR(3)+1
            nerr=nerr+1
...
C
C      IF(nerr.LT.mxerrpri) THEN
C        WRITE(6,*) 'ierr,ieva,izugadc,isprev,ispqrt,jzugsca(ispqrt)'
C     +        ,ierr,ieva,izugadc,isprev,ispqrt 
C      ENDIF
      IF(ierr.EQ.0) THEN
         izug=-1
...
C
C               IF(nerr.LT.mxerrpri) THEN
C                  WRITE(6,*) 'izugmark,istack,icyczug,lzug,ierr'
C     +                       ,izugmark,istack,icyczug,lzug,ierr
C               ENDIF
...
C               IF(nerr.LT.mxerrpri) THEN
C                  WRITE(6,*) 'izugmark,istack,icyczug,isp'
C     +                       ,izugmark,istack,icyczug,isp
C               ENDIF
         IF(isp.LE.0) isp=isp+mxstack
C               IF(nerr.LT.mxerrpri) THEN
C                  WRITE(6,*) 'isp,jzug(isp)',isp,jzug(isp)
C               ENDIF
...
CC  April 1 ,2010 correction for 240Hz , delay=17
CC         IF(k.EQ.0.AND.ispdel<130) jhelrec(ispdel)=jhelcur(istack)
      ENDIF

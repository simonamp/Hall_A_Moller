      REAL FUNCTION ASYMMETQ(ID,NEVADC,ANGL,ANPOW,PTAR,INORM,NORBCM
     +                      ,NOCUT)
C
...
C     +     ,jdif(64),jdifold(64),jhelst(NCDEL)
...
C         IF(ISCA(i).LT.jsca(i)) ierr1=2
      ...
      IF(ierr1.EQ.0.AND.
     +     IABS(iticksc-iticksc0-5).GT.2) THEN
C     +     IABS(iticksc-iticksc0-60).GT.5) THEN
C     +     IABS(iticksc-iticksc0-16).GT.3) THEN
...
C
      ...
      jhelst(1)=jhel
      jcycer(1)=ierr1
      IF(IABS(jtick(1)-jtick(idelcur)-mtick).GT.2) ierr=1
C
C---      Ignore the cuts?
C
      IF(NOCUT.NE.0) THEN
...
C           nsum(i+jhel)=nsum(i+jhel)+jdif(i+jinc,idelcur)
            dsum2(i)=dsum2(i)+(DBLE(jdif(i+jinc,idelcur)))**2
            nsumh(i,jhel)=nsumh(i,jhel)+jdif(i+jinc,idelcur)
            dsumh2(i,jhel)=dsumh2(i,jhel)
     +                    +(DBLE(jdif(i+jinc,idelcur)))**2
C            WRITE(6,*) 'i,jdef ',i,jdif(i+jinc,idelcur)
            NASYM(17+jhel)=nsumh(i,jhel)
         END DO
         nsumh(6 ,jhel)=nsumh( 6,jhel)+jdif(12+jinc,idelcur)
         nsumh(7 ,jhel)=nsumh( 7,jhel)+jdif( 6+jinc,idelcur)
         nsumh(19,jhel)=nsumh(19,jhel)+jdif(19+jinc,idelcur)
         dsum2(6)=dsum2(6)+(DBLE(jdif(12+jinc,idelcur)))**2
         dsum2(7)=dsum2(7)+(DBLE(jdif( 6+jinc,idelcur)))**2
         dsumh2(6,jhel)=dsumh2(6,jhel)+(DBLE(jdif(12+jinc,idelcur)))**2
C
         DO i=1,10
            k=ICHIS(i)
            IF(k.GT.0) THEN
